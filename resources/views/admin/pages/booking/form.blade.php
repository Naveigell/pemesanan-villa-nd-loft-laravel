            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @else
                        ...
                        @endif
                        <form action="{{ route('admin.bookings.payments.update', [$booking, $booking->latestPayment]) }}" class="" method="post">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label>Tipe Pembayaran</label>
                                <input type="text" disabled class="form-control" value="{{ @$booking ? optional($booking->latestPayment)->payment_method->label() : '' }}">
                            </div>
                            <div class="form-group">
                                <label>Bukti Pembayaran</label>
                                <div>
                                    <a href="{{ $booking->latestPayment->payment_proof_image_url }}" class="image-zoom">
                                        <img src="{{ $booking->latestPayment->payment_proof_image_url }}" alt="" style="width: 250px; height: 250px;">
                                    </a>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="payment_status" id="" class="form-control @error('payment_status') is-invalid @enderror">
                                    ...
                                    @foreach(\App\Enums\PaymentStatusEnum::cases() as $case)
                                        <option value="{{ $case->value }}" {{ old('payment_status', @$booking ? optional($booking->latestPayment)->payment_status->value : '') == $case->value ? 'selected' : '' }}>{{ $case->label() }}</option>
                                    @endforeach
                                </select>
